...
|  영수증 양식의 차이    | 양식의 차이가 있더라도 분류가 비교적 쉬운 가게명 / 날짜의 경우 Regex를 사용해 분류, 주소 / 상품명 / 금액은 Claude API를 사용해 분류  |
|  CSV 파일 업로드 시 기존 지출 내역 데이터와 중복될 가능성 존재  | 지출 내역 데이터에 cid(카드 결제)와 bid(현금 결제) 속성을 추가하여 결제 유형을 구분, 업로드된 CSV파일과 DB의 카드 결제 내역을 날짜/금액 기준으로 대조하여 중복 여부 판단, 사용자에게 중복 내역 제거 요청을 보냄으로써 중복 여부에 대한 검토 진행 |
